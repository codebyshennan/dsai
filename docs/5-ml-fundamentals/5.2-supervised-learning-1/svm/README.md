# Support Vector Machines (SVM)

**After these lessons, you can train SVMs for classification and regression, apply kernel tricks for non-linear boundaries, and tune C and γ.**

## Overview

SVMs find the maximum-margin hyperplane separating classes; kernels project data to higher dimensions where linear separation becomes possible. Prerequisites: [5.1 Intro to ML](../../5.1-intro-to-ml/README.md).

## Lesson Path

1. [Introduction](1-introduction.md)
2. [Math & kernels](2-math-kernels.md)
3. [Implementation](3-implementation.md)
4. [Advanced topics](4-advanced.md)
5. [Applications](5-applications.md)
